...
class DeliveryTests(unittest.TestCase):
    ...
    def test_delivered_profile_receipt_and_action_metadata(self):
        ...
        self.assertEqual(entry["Action"],"com.arkamax.ulanzi.imageslide.slideshow");self.assertEqual(entry["Plugin"],{"Name":"Image Slideshow","UUID":"com.arkamax.ulanzi.imageslide","Version":"0.7.3"})
    ...
    def test_profile_version_update_preserves_identity_and_other_members(self):
        with tempfile.TemporaryDirectory() as td:
            profile=Path(td)/"profile.ulanziDeckProfile";shutil.copyfile(ROOT/"ImageSlide.ulanziDeckProfile",profile)
            _,before=tool.read_archive(profile);names=before.namelist();selected=next(item for item in tool.candidates(before) if item["action"]==tool.ACTION_UUID);unchanged={name:before.read(name) for name in names if name!=selected["manifest"]}
            tool.set_plugin_version(profile,"9.8.7");first=profile.read_bytes();tool.set_plugin_version(profile,"9.8.7")
            self.assertEqual(profile.read_bytes(),first);_,after=tool.read_archive(profile);self.assertEqual(after.namelist(),names)
            self.assertEqual({name:after.read(name) for name in unchanged},unchanged)
    ...
    def test_helper_prefers_v2_and_persists_bounded_codes(self):
        ...
        self.assertIn("CurrentMatchesPatchedReceipt",helper);self.assertIn("NormalizedPatchedPage",helper);self.assertIn("CenterFingerprint",helper);self.assertIn("centerActionFingerprintSha256",helper);self.assertIn("@($params.PSObject.Properties).Count",helper);self.assertIn("fingerprintedCandidates",helper);self.assertIn("legacyCandidates",helper)
...
''',encoding="utf-8")
    ...
    def test_diagnostic_record_has_privacy_bounded_fields_only(self):
        ...
        self.assertIn("timestampUtc",record);self.assertIn("category",record);self.assertIn("setupKey",record);self.assertIn("setupActionIdSha256",record)
    ...
    def test_store_metadata_and_manifest_declare_one_universal_listing(self):
        ...
        for pattern in ("*.png binary","*.zip binary","*.ulanziDeckProfile binary"):self.assertIn(pattern,attributes)
        store=json.loads((ROOT/"store.json").read_text(encoding="utf-8"))
        self.assertEqual(store["cover"],"resources/store/cover.png")
        self.assertEqual(store["screenshots"],["resources/store/banner.png"])
        ...
        cover_width,cover_height=tool.PNG_DIMS((ROOT/store["cover"]).read_bytes());banner_width,banner_height=tool.PNG_DIMS((ROOT/store["screenshots"][0]).read_bytes())
        self.assertAlmostEqual(cover_width/cover_height,2,delta=0.002);self.assertAlmostEqual(banner_width/banner_height,3/2,delta=0.002)
        self.assertGreaterEqual(cover_width,1200);self.assertGreaterEqual(banner_width,1200)
    ...
    def test_release_builder_pins_only_the_three_runtime_targets(self):
        ...
        source=(ROOT/"tools"/"build_release.py").read_text(encoding="utf-8");self.assertNotIn("npm install",source);self.assertIn("entry[\"integrity\"]",source);self.assertIn("pe_machine",source);self.assertIn("macho_cpu",source);self.assertIn('newline="\\n"',source)
    ...
    def test_packages_and_checksum_manifest_read_back(self):
        ...
        with zipfile.ZipFile(ROOT/(plugin_root+".zip")) as archive:
            ...
            manifest=json.loads(archive.read(plugin_root+"/manifest.json"));self.assertEqual(manifest["Version"],"0.7.3");self.assertEqual(manifest["Author"],"Santiago P\u00e9rez")
            for name in names:
