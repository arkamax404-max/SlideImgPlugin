...
''',encoding="utf-8")
    ...
    def test_store_metadata_and_manifest_declare_one_universal_listing(self):
        ...
        for pattern in ("*.png binary","*.jpg binary","*.zip binary","*.ulanziDeckProfile binary"):self.assertIn(pattern,attributes)
        store=json.loads((ROOT/"store.json").read_text(encoding="utf-8"))
        self.assertEqual(store["cover"],"resources/store/cover.jpg")
        ...
        cover_width,cover_height=tool.JPEG_DIMS((ROOT/store["cover"]).read_bytes());banner_width,banner_height=tool.PNG_DIMS((ROOT/store["screenshots"][0]).read_bytes())
        self.assertEqual((cover_width,cover_height),(1200,630));self.assertAlmostEqual(banner_width/banner_height,3/2,delta=0.002)
        self.assertGreaterEqual(banner_width,1200)
