...
(() => {
  ...
  const state={uuid:ACTION_UUID,key:query.get("key")||"",actionid:query.get("actionid")||"",settings:{folderPath:"",intervalSeconds:10,loop:true,sort:"name"},socket:null};
  ...
  function paintSettings(){el("folder").value=state.settings.folderPath||"";el("interval").value=state.settings.intervalSeconds;el("loop").checked=state.settings.loop;el("sort").value=state.settings.sort;}
  ...
  function save(){const intervalSeconds=Number(el("interval").value);if(!Number.isFinite(intervalSeconds)||intervalSeconds<5){paintStatus({level:"error",message:"Interval must be at least 5 seconds."});return;}state.settings={folderPath:state.settings.folderPath,intervalSeconds,loop:el("loop").checked,sort:el("sort").value};sendToPlugin({type:"updateSettings",settings:state.settings});}
  ...
  for(const id of ["interval","loop","sort"])el(id).addEventListener("change",save);
